...
namespace MageINIC\Faq\Model\ResourceModel\CategoryAbstract;
...
abstract class AbstractCollection extends CollectionAbstractCollection
{
     ...
     */
    protected function performAfterLoad(mixed $tableName, mixed $linkField)
    {
        $linkedIds = $this->getColumnValues($linkField);
        if (count($linkedIds)) {
            $connection = $this->getConnection();
            $select = $connection->select()->from(['mageinic_faq_category_store' => $this->getTable($tableName)])
                ->where('mageinic_faq_category_store.' . $linkField . ' IN (?)', $linkedIds);
     ...
     * @param bool $withAdmin
     */
    protected function performAddStoreFilter(mixed $store, bool $withAdmin = true)
    {
